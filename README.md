...
In this case, you can rebase interactively (`git rebase --interactive master`). You'll get a choice about which commits you want − presumably just those commits which you introduced in *your branch*, rather than any commits that you thought belonged to master. If the commits in master have changed, you'll be offered some of the old master's commits, as though they're part of your branch. If you think that the new `master` still has much the same functionality as your original base (old master), then `drop` those outdated base commits `pick` all of *your* commits that were actually relevant to implementing your widget.
